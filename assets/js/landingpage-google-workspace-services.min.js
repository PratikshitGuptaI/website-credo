 ...
 */
{
	...
	class Item {
      ...
      // for (var i = 0; i < this.DOM.labelLetters.length; i++) {
      //   this.DOM.labelLetters[i].className += ' background-check';
      // }
			this.initEvents();
		}
		initEvents() {
			this.mouseenterFn = () => this.mouseTimeout = setTimeout(() => {
				...
				anime({
					...
					delay: (t,i) => i*15,
					translateY: [
						{value: -15, duration: 150, easing: 'easeInQuad'},
						{value: [15,0], duration: 150, easing: 'easeOutQuad'}
					],
					opacity: [
...
const pagesWithSliders = [
  '/services/ux-capability-services.html',
  '/CNT/websites/cloudnow-website-v2/dist/services/ux-capability-services.html',
];

const pagesWithCounters = [
  '/services/medical-app-services.html',
  '/services/application-development-services.html',
];

$(function() {
  if (pagesWithSliders.indexOf(pathname) >= 0) {
    $('.app-slider').slick({
      centerMode: true,
      speed: 1000,
      slidesToShow: 3,
  ...
  // onscroll animating counter script
  if (pagesWithCounters.indexOf(pathname) >= 0) {
    let a = 0;
    $(window).scroll(function() {
  ...
  const alterClass = function() {
    ...
    if (ww < 800) {
      $('.modal').removeClass('mobile__form--resize');
      $('.modal-header').removeClass('modal-header--resize');
    }
  };
  ...
  $(window).scroll(function() {
    ...
    if (scroll >= 300) {
      $('.mobile__menu--btn').removeClass('hidden');
      // alert('test');
    } else {
      $('.mobile__menu--btn').addClass('hidden');
    }
  });

  $('.mobile__menu--btn').click(function() {
    $('.modal').removeClass('mobile__form--resize');
    $('.modal-header').removeClass('mobile__form--resize');
  });

  let curOffset;
  let oldOffset;
  oldOffset = $(window).scrollTop();
  const $el = $('.mobile__menu--btn'); // cache jquery ref
  ...
  $('.open__popup').click(function() {
    $('.leftSide__content').toggleClass('no-visible');
    $('.show__form').toggleClass('active');
  });
  $('.return__content').click(function() {
    $('.leftSide__content').toggleClass('no-visible');
    $('.show__form').toggleClass('active');
  });
});
...
if (loc === 'cloudnowtech.com' || loc === 'www.cloudnowtech.com') {
  gasServicesURL =
    'https://script.google.com/macros/s/AKfycbwkIZt5QICuK1VTkWQiJavlax7pDBrCsKatJfLe3yCyCffLNbMqDbfU5D2s-q_Klhig/exec';
} else {
  gasServicesURL =
    'https://script.google.com/macros/s/AKfycbwkIZt5QICuK1VTkWQiJavlax7pDBrCsKatJfLe3yCyCffLNbMqDbfU5D2s-q_Klhig/exec';
}
...
$(document).ready(function() {
  $('#servicesModal .remove').click(function() {
    setTimeout(function() {
      $('#servicesModal').remove();
    }, 400);
  });
});
...
$(document).on('keypress', 'input.phone', event => {
...
// services type
const cloudAdvisoryServices = 'cloud-advisory-services';
const applicationDevelopmentServices = 'application-development-services';
const cloudManagedServices = 'cloud-managed-services';
const devopsServices = 'devops-services';
const cloudMigrationServices = 'cloud-migration-services';
const gcpManagedServices = 'gcp-managed-services';
const googleWorkspaceServices = 'google-workspace-services';
let servicesName;

$('#cloudAdvisoryBtn').click(function() {
  servicesName = cloudAdvisoryServices;
});
$('#applicationDevelopmentBtn').click(function() {
  servicesName = applicationDevelopmentServices;
});
$('#cloudManagedBtn').click(function() {
  servicesName = cloudManagedServices;
});
$('#cloudManagedBtn1').click(function() {
  servicesName = cloudManagedServices;
});
$('#devopsBtn, #devopsBtn1').click(function() {
  servicesName = devopsServices;
});
$('#cloudMigrationBtn').click(function() {
  servicesName = cloudMigrationServices;
});
$('#gcpManagedBtn, #gcpManagedBtn1').click(function() {
  servicesName = gcpManagedServices;
});

$('#googleWorkspaceBtn').click(function() {
  servicesName = googleWorkspaceServices;
});
...
// get-in-touch
// form submission for compaignForm
$('#servicesForm').validate({
  ...
  submitHandler() {
    $('#servicesModal .btn-submit').html("<i class='fa fa-circle-o-notch fa-spin'></i> Submitting");
    $('#servicesModal .btn-submit').attr('disabled', 'disabled');
    const name = $('#name')
      .val()
      .trim();
    const servicesEmail = $('#servicesEmail')
      .val()
      .trim();
    const phone = $('#phone')
      .val()
    ...
    formData.append('Email', servicesEmail);
    formData.append('Services', services);
    formData.append('Selected Services', selectServices);
    // google sheet tab name
    formData.append('Type', 'services_form');
    $.ajax({
      type: 'POST',
      url: gasServicesURL,
      data: formData,
      success() {
        $('#servicesModal .btn-submit').html('send');
        $('#servicesModal .btn-submit').removeAttr('disabled');
...
$('#enquiryForm').validate({
  ...
  submitHandler() {
    $('#enquiryForm .btn-submit').html("<i class='fa fa-circle-o-notch fa-spin'></i> Submitting");
    $('#enquiryForm .btn-submit').attr('disabled', 'disabled');
    const name = $('#enquiryName')
      .val()
      .trim();
    const servicesEmail = $('#enquiryEmail')
      .val()
      .trim();
    const servicesPhone = $('#enquiryPhone')
      .val()
      .trim();
    const message = $('#enquiryComments')
      .val()
      .trim();
    const services = servicesName;
    ...
    const formData = new FormData();
    formData.append('Name', name);
    formData.append('Phone', servicesPhone);
    ...
    $.ajax({
      ...
      data: formData,
      success() {
        $('#enquiryForm .btn-submit').html('send');
        $('#enquiryForm .btn-submit').removeAttr('disabled');
        window.location.replace('thank-you.html');
        // swal('Thanks', 'Someone from CloudNow will be in touch!', 'success').then(() => {
        //   document.getElementById('servicesForm').reset();
        //   $('#servicesModal').modal('hide');
